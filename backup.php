...
function __backup_mysql_database($params)
{
    ...
    $backup_file_name = "bkups/KPCL.sql";
...
$para = array(
    ...
    'db_uname' => 'root',  //user
    'db_password' => '', //pass
    'db_to_backup' => 'sales', //database name
...
$QQ = mysql_query("SELECT * FROM onlineUPdate WHERE id='1'");
    ...
    if ($date!=$dated) {
        require_once "send-email-php/phpmailer/phpmailer.php";
                ...
                $bodyContent = '<h1>Kwasi Oppong Company Limited</h1>';
                $bodyContent .= '<p>Sql back up on <b>'.date("js F, Y").'</b></p>';

                $mail->Subject = 'Email from Localhost by KPCL';
                $mail->Body    = $bodyContent;
                $mail->AddAttachment("bkups/KPCL.sql");
