...
class PDF extends FPDF{
    ...
    function Header(){
      ...
      $this->setFont('Arial','B',12);
				...
				$brachID= next($explode);

$qq=mysql_query("SELECT * FROM braches WHERE id='$brachID'");
...
$differentiator=$get['differentiator'];
      ...
      $this->Cell(300,10,'INVENTORIES AND SALES OF  '.mb_strtoupper($branchName).' BRANCH  - '.mb_strtoupper($Location).' ',0,1,'C');
      $this->Cell(300,10,'TOWARD  '.mb_strtoupper($differentiator).' '.mb_strtoupper($itemName).' ',0,1,'C');
      $this->Ln(5);
...
$tSALES_AMT="";
$tTOTAL_AMT="";
$query=mysql_query("SELECT * FROM salesandsupplies WHERE branch='$brachID' AND itemID='$itemId' AND active='yes' ORDER BY invent_Date ASC");
